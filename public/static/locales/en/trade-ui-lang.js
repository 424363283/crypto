var tradeUiLang = {
  ...
  '买方及卖方在下个资金时段要交换的资金费率。费率为正，买方支付卖方。费率为负，卖方支付买方。{more}':
    'Funding rate between the two parties will be changed at the next time. Positive rate, the buyer will pay the seller, the negative rate, the seller will pay the buyer. {more}',
  了解更多: 'Learn More',
  ...
  '风险提示：交易前请注意查看净值价格，该产品在极端行情下会存在价格趋近于归零的风险，请投资者注意控制风险。':
    'Risk warning: Please check the price before trading, the product is subject to the risk of price convergence to zero in extreme market conditions, so investors are advised to control the risk.',
  '1.“3”为追踪标的资产涨跌幅的3倍，“L”为做多，“S”为做空。当标的资产上涨1%时，3L上涨3%，3S下跌3%。当标的资产下跌1%时，3L下跌3%，3S上涨3%。':
    '1. "3" stands for 3x rise and fall of the underlying asset, "L" stands for long and "S" stands for short. For example, if the price of BTC rises 1%, then BTC3L will rise 3% and BTC3S will fall 3%.',
  '2.每天0点（UTC+8）会进行定时再平衡，更新净值基准值和标的资产基准值。':
    '2. To ensure that the ratio of the combined leverage portfolio will not deviate much from the agreed one, the system will rebalance positions at 00:00:00 (UTC+8) every day (apart from exceptional circumstances).',
  '3.一个再平衡周期内，标的资产涨跌幅触发15%，将进行阈值再平衡，更新净值基准值和标的资产基准值，防止爆仓。':
    '3. In case of sharp fluctuations, if compared with the last rebalancing, the change of the underlying asset exceeds the given threshold (initially set to 15% for 3 times long and short), YMEX will rebalance the positions to control the risks.',
  '4.净值为该ETF产品的实际价值，所以请用户在参与交易时价格不要偏离净值过大，避免造成不必要的损失。':
    '4. The net value the actual value of the Leveraged Tokens, so users should trade with caution when the transaction price deviates too much from the net value.',
  '5.当ETF产品的净值低于0.1时会触发合并机制，该ETF产品的净值变为合并前的10倍，同时该ETF产品的数量也会变为合并前的1/10，用户总资产价值不会受任何影响，以提高价格变化灵敏度，优化交易体验。':
    '5. To improve the sensitivity of price changes and optimize the trading experience, when the net value of a Leveraged Tokens is lower than a certain threshold, YMEX will merge the shares of the product. After merging, the net value will be up 10 times but the quantity will become 1/10, the total assets of the investor will not be affected. ',
  社交媒体: 'Socials',
  数据来自: 'Data from',
  官网: 'Website',
  浏览器: 'Block Explorers',
  ...
  '淨值=上次調倉時淨值*[1±{lever}*(標的資產最新成交價-標的資產上次調倉時價格)/標的資產上次調倉時價格*100%]':
    'Net Asset Value = Net asset value at last correction * [1±{lever}* (last trade price - target price at last correction) / target price at last correction * 100 %]',
  净值: 'NAV',
  '每日00:00:00(UTC+8)收取，管理费体现在净值中，用户的持仓并不会减少':
    "Management fees are charged by 00:00:00 (UTC+8) every day. The management fee is reflected in the net value, and the user's holdings will not reduced.",
  管理费: 'Management Fee',
  '24H最高': '24H High',
  '24H最低': '24H Low',
  '24H成交量': '24H Volume',
  ...
  '买方及卖方在下个资金时段要交换的资金费率。资金费率周期内(每 8 小时)每分钟计算一次溢价指数，并将溢价指数以时间加权平均的方式应用于计算资金费率。 {more}':
    'Funding rate exchanged between buyers and sellers. During the funding rate cycle (every 8 hours), the premium index will be calculated every minute and the time-weighted average value will be used to calculate the funding rate. {more}',
  ...
  AI策略: 'AI Strategy',
  手动创建: 'Manual',
  'AI推荐参数，是通过行情数据回测产生的，请您谨慎使用！':
    'AI strategy parameters are generated through market data backtesting, use with caution !',
  ...
  '每格利润率必须大于{profit}%，请减少网格数量或增大价格区间':
    'Profit margin per grid must be more than {profit}%, reduce the number of grids or increase the price range',
  可用: 'Available',
  起投: 'Minimum',
  ...
  '网络服务器连接断开,请及时刷新页面。':
    'The network server connection has been disconnected. Please refresh the page promptly.',
  '{name} 刚刚平仓了一个 {symbol} 合约仓位，获得了 {profit} 的盈利':
    '{name} just closed a {symbol} contract position and gained {profit} profit.',
  网络稳定: 'Network Stable',
  ...
  张: 'Cont',
  '成交价格与净值偏离过大时，请谨慎交易':
    'When the transaction price deviates too much from the net value, please trade with caution',
  價格: 'Price',
  累计: 'Sum',
  '标记价格由实时指数价格和即将到来的资金费率决定，反映该合约当前的合理价格。标记价格用于强平触发。{more}':
    'Mark Price is calculated based on the index price. The Price Index is a bucket of prices from the major Spot market exchanges, weighted by their relative volume. {more}',
  ...
  委托类型: 'Type',
  止盈止损: 'TP/SL',
  '由于行情变动快，止损触发价不宜设置离强平价格过近，避免触发失败。':
    'Due to the rapid changes in the market, the stop loss trigger price should not be set too close to the liquidation price to avoid trigger failure.',
  止盈: 'TP',
  ...
  '预估强平价为下单前预估的仓位强平价格，仅供参考。仓位的实际强平价取决于开仓均价、浮动盈亏和保证金。':
    'The estimated liquidation price is the estimated position liquidation price before placing an order, which is for reference only. The actual liquidation price of a position depends on the average opening price, floating profit, loss and margin.',
  预估强平价: 'Estimated Liq. Price',
  '差距为正，代表标记价格上涨会导致爆仓。差距为负，代表标记价格下跌会导致爆仓。':
    'A positive gap means that an increase in the mark price will lead to liquidation. A negative gap means that a drop in the mark price will lead to liquidation.',
  ...
  '当{type}达到{price}，将会触发限价单委托，委托价格为{orderPrice}。预计盈利为{income}（收益率为{rate})':
    'When {type} reaches {price}, the limit order will be triggered, limit price {orderPrice}. Estimated profit {income} (ROI {rate})',
  '当{type}达到{price}，将会触发市价委托。预计盈利为{income}（收益率为{rate})':
    'When {type} reaches {price}, the market order will be triggered. Estimated profit is {income} (ROI {rate})',
  '当{type}达到{price}，将以{orderPrice}委托平仓，预估收益为{income}（{rate})':
    'When the {type} reaches {price}, the position will be closed at {orderPrice}, and the estimated profit is {income}（{rate})',
  '市价止盈/止损能够最大可能的保证订单实时成交，但也需面临成交价格与设置触发价格有偏差的风险。':
    'Market orders (TP/SL) guarantee real-time order execution, but there is a risk of deviation between the transaction price and the set trigger price.',
  '限价止盈/止损订单能够保证以指定价格成交，但也需面对订单未能成交的风险。':
    'Limit orders (TP/SL) guarantee trading at a specified price, but there is also the risk of not executing the order.',
  '当{type}达到{price}时，将会触发市价止盈委托平仓当前仓位。预计盈利为{income}是 (收益率为{rate})':
    'When {type} reaches {price}, the current order will close at market price. Estimated profit {income} (ROI {rate})',
  ...
  '预期盈亏为平仓的估算盈亏，手续费和实际成交的价格差异会使得预估值和实际值有较大差异，请仅供参考。':
    'Expected profit and loss is the estimated profit and loss when closing the position. The difference between the transaction fee and the actual transaction price will result in a large difference between the estimated value and the actual value. Please use it for reference only.',
  '市价止盈止损委托根据触发价格估算预期盈亏。限价止盈止损根据限价价格估算预期盈亏。':
    'Market price stop profit and stop loss order estimates the expected profit and loss based on the trigger price. Limit price, stop profit and stop loss estimate the expected profit and loss based on the limit price.',
  ...
  '仅允许设置一个应用于整个仓位的止盈止损委托。当价格达到止盈或止损触发价格时，将以市价全部平仓。':
    'This setting will allow you to place a single TP/SL order per position. When the TP/SL trigger price is reached, all positions will be closed at the market price.',
  '允许设置多个应用于部分仓位的止盈止损委托。达到部分止盈或止损触发价格时，将通过市价单或限价单平仓对应的止盈或止损委托数量。':
    'This setting will allow you to place multiple TP/SL orders per position. When a part of the TP/SL trigger price is reached, the corresponding TP/SL order quantity will be closed by the market order or limit order.',
  设置止盈止损: 'Set TP/SL',
  ...
  '最新成交价达到{price}时，追踪出场订单将被激活':
    'When the latest transaction price reaches {price}, the trailing stop order will be activated',
  '最新成交价从最优价格回撤{price}时，将触发追踪出场，执行市价平仓。':
    'When the latest transaction price retraces from the best price by {price}, it will trigger the trailing stop and execute a market close.',
  按价格: 'By Price',
  '根据距离最高/低点的价差计算触发价格':
    'Calculated trigger price based on the price difference from the highest/lowest point',
  按比例: 'By ratio',
  ...
  做多方向的激活价格必须大于开仓价和最新价:
    'The activation price for long positions must be higher than the opening price and the latest price',
  做空方向的激活价格必须小于开仓价和最新价:
    'The activation price for short positions must be lower than the opening price and the latest price',
  回撤价差: 'Callback Price',
  开仓价: 'Entry',
  强平价格: 'Liq.Price',
  激活价格: 'Activation price',
  '激活价格是追踪出场的激活条件，当市场最新成交价达到或超过激活价格，追踪出场委托就会被激活。激活后系统开始计算追踪出场的实际触发价格。':
    "The activation price is the condition for activating the trailing stop. When the market's latest transaction price reaches or exceeds the activation price, the trailing stop order will be activated. After activation, the system starts calculating the actual trigger price of the trailing stop.",
  '当最新成交价从最优价格回撤时，将出发追踪出场，执行市价平仓。':
    'When the latest transaction price retraces from the best price, it will trigger the trailing stop and execute a market close.',
  ...
  '回撤幅度是计算实际触发价格的要条件。实际价格会根据历史最高/最低和回撤幅度计算得出。例如，设置回撤幅度 5%的追踪出场，历史最高价20,000，则当前实际触发市价平仓的价格为20,000*(1-5%)=19,000。':
    'Retracement magnitude is a condition for calculating the actual trigger price. The actual price is calculated based on the historical highest/lowest and the retracement magnitude. For example, setting a retracement magnitude of 5% for the trailing stop, with a historical high price of 20,000, then the current actual trigger price for market close is 20,000*(1-5%)=19,000.',
  请输入比例: 'Enter a ratio',
  ...
  '槓桿會放大您的最大購買力，如果10X做多，當前價格若上漲1%，盈利將達到10%；當前價格下跌1%，虧損將達到10%':
    'Leverage will amplify your maximum purchasing power. When you buy 10X, if the current price increases by 1%, the profit will reach 10%, if the current price decreases by 1%, the loss will reach 10%.',
  新手建議使用低倍槓桿: 'Beginners are advised to use low leverage',
  '填入價格/數量': 'Enter price/qty',
  '填入你預期的開倉價格，如果市價單則會以最優價成交':
    'Enter your expected opening price. If you place a market order, it will be filled at the best price.',
  '填入你預期的開倉數量，其中最大可開數量為當前槓桿下的最大購買力':
    'Enter the number of positions you expect to open, where the maximum number that can be opened is the maximum purchasing power under the current leverage.',
  '根據您對漲跌的判斷在填寫價格和數量之後，可選擇開多或是開空。':
    'After entering the price and quantity based on your assessment of increase or decrease, you can choose to open a buy or sell position.',
  ...
  '挂单成功后，将会从礼金钱包扣除最多{val}%的保证金（如果可用），手续费将在开仓成功时从红包钱包扣除（如果可用）':
    'After the order is successfully placed, a maximum of {val}% margin will be deducted from the trial fund balance (if available), and the trading fee will be deducted from the voucher balance (if available) when the position is successfully opened',
  体验券: 'Trial Fund',
  到期: 'Expired',
  ...
  'ETF杠杆代币波动较大，请在交易前注意查看净值价格，并仔细阅读':
    'Leveraged Tokens are volatile, please check the net value and read carefully before trading',
  ...
  '执行{baseCoin}/{quoteCoin}现货网格需要买入一定数量的{baseCoin},确定创建后将以市场最优价格执行下单，下单过程中价格波动可能产生价差。':
    'Execute {baseCoin}/{quoteCoin} spot grid, you need to buy a certain amount of {baseCoin}. After confirming the creation, the order will be executed at the best price in the market. Price fluctuations may cause price differences during the order placement process.',
  启用: 'Enable',
  关闭: 'Turn off',
  排行榜: 'Leaderboard',
  免费复制: 'Copy for Free',
  ...
  生效时间: 'TIF',
  'GTC (有效直到取消): 此种订单将持续有效，直到完全成交或被取消。':
    'GTC (Good Till Canceled): Remain in effect until fully filled or canceled',
  'IOC (立即成交或取消): 此种订单将会立即成交全部或部分订单，并且取消剩余未成交的部分。':
    'IOC (Immediate or Cancel): Fill all or part of the order immediately and cancel the remaining unfilled part',
  'FOK (全部成交或取消): 此种订单必须立即全部成交，否则将被全部取消。':
    'FOK (Fill or Kill): Must be filled immediately, otherwise it will be canceled',
  指南: 'Guide',
  ...
  市价卖出: 'Sell (Short）',
  温馨提示: 'Reminder',
  '您的合约账户内存有：{amount} {cryoto} 可用体验金':
    'Your contract account contains: {amount} {cryoto} bonus available.',
  ...
  单向持仓: 'One-Way Mode',
  '在计算最大可开数量时将不考虑您的开仓损失。':
    'The amount lost when opening an order will not be included in the calculation of the max opening amount',
  起始保证金: 'Initial Margin',
  '起始保证金有时会小于下单所需的保证金。下单所需保证金=起始保证金+开仓亏损，其中开仓亏损考虑了盘口最新情况于标记价格之前的差异':
    'The initial margin may not be equal to the actual margin required to place an order. Actual margin required = initial margin + open loss, where open loss reflects the additional cost due to the difference between the order price and mark price.',
  ...
  '订单若不能立即成交则未成交的部分立即取消。':
    'Fill all or part of the order immediately and cancel the remaining unfilled part',
  ...
  '选取过高的杠杆倍数[10倍] 会增加强行平仓风险，请注意自身的风险等级。':
    'Higher leverage, such as [10x], increases liquidation risk. Always manage your risk.',
  ...
  '全仓模式：保证金资产相同的全仓仓位共享该资产的全仓保证金。在强平事件中，交易者可能会损失全部该保证金和该保证金资产下的所有全仓仓位':
    'Cross Position Mode: All cross positions under the same margin asset share the same asset cross margin balance. In the event of liquidation, your assets full margin balance along with any remaining open positions under the asset may be forfeited.',
  '逐仓模式：一定数量保证金被分配到仓位上。如果仓位保证金亏损到低于维持保证金的水平，仓位将被强平。在逐仓模式下，您可以为这个仓位添加和减少保证金。':
    'Isolated Position Mode: Manage your risk on individual positions by restricting the amount of margin allocated to each. If the margin ratio of a position reached 100%, the position will be liquidated. Margin can be added or removed to positions using this mode.',
  回撤价格: 'Callback Price',
  '当市场价与标记价格偏离超过{n}%时，下单可能失败。':
    'When the market price deviates from the marked price by more than {n}, the order may expire or be partially filled.',
  '不再展示，您可在【偏好设置】中重新设置。': 'Disable order confirmation, you can reset it in [Preferences].',
  '模拟资产余额低于最低额度({value} {crypto})时，才可再次领取，无领取次数限制。':
    'When demo asset balance is lower than the minimum amount({value} {crypto}), you can reset it anytime and without limit.',
  ...
  '限价买单价格应小于或等于（1 + 价格上限比例）* 合约当前标记价格。':
    'The buy price of limit orders should be less or equal to (1 + cap ratio )* current Mark Price of the contract.',
  限价卖单价格下限比例: 'Limit Order Price Floor Ratio',
  '限价卖单价格应大于或等于（1 - 价格下限比例）* 合约当前标记价格。':
  ...
  '每个合约的最大挂单数量（包括条件单）。每位用户每个合约下，可以同时处于挂单状态的条件单（市价止盈止损）上限为{number}单。':
    'Maximum number of open orders (including conditional orders) per contract. The upper limit of the conditional orders (market price TP & SL) for each user and contract is {number} order(s).',
  ...
  '请填写订单价值，可通过调整杠杠来修改下单所需保证金。':
    'Enter your desired order value. You can modify the required margin by adjusting the applied leverage.',
  按保证金下单: 'Order by Cost',
  '请填写订单成本，包括起始保证金及开平仓手续费。修改杠杆不会改变成本。':
    'Enter your order cost that‘s inclusive of the initial margin, fees to open, and fees to close. Modifying the leverage will not change your order cost.',
  '请注意：您的订单数量按您填写的开仓成本计算。请注意，如果出现极端市场波动，您的订单委托可能失败。':
    "Your order qty will be calculated based on the order cost you've entered. Please note that your order may fail in the event of extreme market fluctuations.",
  '当{pricetype}触达{price}时，将会触发{type}委托，预期盈亏将会是{value}':
    'When {pricetype} reaches {price}, {type} order will be triggered, and the estimated profit and loss will be {value}',
  市价单: 'Market Price',
  开多: 'Long',
  开空: 'Short',
  ...
  '双向持仓模式下，一个合约可允许同时持有多空两个方向的仓位模式。 调整对所有合约统一生效。该设置仅对U本位合约生效。':
    'In hedge mode, one contract position allows to hold long position and short position. Only applies to USDT-M contracts',
  '双向持仓模式下，一个合约可允许同时持有多空两个方向的仓位模式。 调整对所有合约统一生效。该设置仅对币本位合约生效。':
    'In hedge mode, one contract position allows to hold long position and short position. Only applies to COIN-M contracts',
  '若U本位合约在持仓或者挂单，不允许调整仓位模式。仓位模式调整对所有合约统一生效。该设置仅对U本位合约生效。':
    'Position mode cannot be adjusted while holding or placing orders. Applicable to all USDT-M contracts',
  '若币本位合约在持仓或者挂单，不允许调整仓位模式。仓位模式调整对所有合约统一生效。该设置仅对币本位合约生效。':
    'Position mode cannot be adjusted while holding or placing orders. Applicable to all COIN-M contracts',
  仓位模式: 'Position Mode',
  系统默认钱包: 'Default Wallet',
  去交易: 'Trade',
  '此钱包为系统默认钱包，当同意合约协议时，系统自动创建的钱包；此钱包支持修改头像和名称。':
    'This wallet is the default wallet, a wallet automatically created by the system when agreeing to the contract agreement. Modifying the avatar and name is supported.',
  '{wallet}为体验金交易专用钱包，活动派发的体验金将会发放至该钱包；若使用体验金交易需切换至该钱包进行操作，且交易平仓已实现盈利部分可转出；体验金钱包头像与名称不支持修改。':
    "{wallet} is a wallet dedicated to bonus transactions. Bonuses are distributed to this wallet. If you use the bonus for trading, you need to switch to this wallet to trade, and the transaction's profit after closing the order can be withdrawn. Modifying the avatar and name is not supported.",
  '获得了体验金后，在未有持仓的前提下，永续合约账户任何资金的划转转出，都会导致体验金失效。':
    'After receiving the bonus without holding any positions, any transfer out of the perpetual contract account will the bonus to become invalid.',
  '有体验金且有持仓时，体验金钱包不可操作划转转出。':
  ...
  '下单被拒绝，下单数量超过当前杠杆最大可开数量':
    'The order was rejected because the order quantity exceeds the current maximum allowable leverage.',
  ...
  '下单失败，当前最新价格与标记价格价差已超出设定阈值':
    'Failed to place an order, the difference between the latest price and the mark price has exceeded the set threshold',
  子钱包功能维护中: 'The sub wallet function is under maintenance',
  '风险限额变更，限制开新仓':
    'The risk limit of this trading pair has changed. Close the position first and get a new limit to open a new position.',
  编辑: 'Edit',
  资金费率: 'Funding',
  通知设置: 'Notification Settings',
  '开启以下通知开关后，您将通过邮箱、站内信、APP推送收到通知。':
    'After turning on the following notification setting, you will receive notifications via email, messages, and APP notification.',
  止盈止损触发通知: 'TP SL trigger notification',
  '每位用户每日上限发送25条；委托类型包括限价止盈/止损、市价止盈/止损、追踪委托五种。':
    'Each user can send up to 25 messages per day, the order types include Limit orders (TP/SL) , Market orders (TP/SL) , and Trailing orders.',
  ...
  '{APP_NAME}会在支付的资金费率 触及时通知您。':
    '{APP_NAME} will notify you when the Funding rate is expected to be paid',
  追加保证金通知: 'Margin call notice',
  '{APP_NAME}会在您持仓仓位的触发强平 前通知您，便于您及时追加仓位保证金，避免仓位被强平。':
    '{APP_NAME} will notify you prior to triggering liquidation of your positions, allowing you to add margin to your positions in time to avoid liquidation.',
  ...
  标记: 'Mark',
  '您可以在开仓前设置止盈止损，开仓后止盈止损才会生效。您也可以选择最新价或者标记价来触发止盈止损。':
    'You can set the TP and SL before opening the position, and the TP and SL will take effect after opening the position. You can also choose the last price or mark price to trigger TP/SL orders.',
  高级: 'Advanced',
  模拟交易账户: 'Demo account',
  '您当前正在使用{walletName}，点击可添加或切换其他子钱包':
    'You are currently using {walletName}, click to add or switch other sub-wallets',
  体验金钱包: 'Bonus wallet',
  止盈止损触发设置: 'TP/SL Settings',
  '按价格(USDT)：止盈止损的触发价格。': 'By Price (USDT): TP/SL trigger price',
  '按收益率(%)：预期收益率，计算出止盈止损的触发价格。':
    'By ROI (%): Expected ROI, calculate the trigger price of TP/SL.',
  '按盈亏(USDT)：预计盈亏，计算出止盈止损的触发价格。':
    'By PNL(USDT): Expected PNL, calculate the trigger price of TP/SL.',
  按收益率: 'By ROI (%)',
  按盈亏: 'By PNL',
  盈亏: 'PNL',
  ...
  修改保证金: 'Modify Margin',
  是否递延: 'Deferral',
  '简易合约存在交割日期，持仓会在每天5:55:00（UTC+8）自动平仓。递延是指到期不自动平仓，以收取递延费形式继续持仓，直到到达递延最大次数为止。当到达递延最大次数后，仓位会被自动平仓。':
  ...
  '指标数量超过10个，请删除一些指标': 'The number of indicators exceeds 10. Please remove some indicators.',
