...
import { CURRENT_TAB, CURRENT_VIEW, store } from '../../../store';
...
const TopOptions = ({ external = true, children }: { external?: boolean, children?: React.ReactNode | JSX.Element[]}) => {
  ...
  const LeftOptions = ({ className }: { className?: string }) => {
    return (
      <>
        {HEADER_TABS_CONFIG.filter(tabItem => tabItem.id !== CURRENT_TAB.LITE || !isMobile || enableH5Lite ).map(item => {
          return (
            <div
              key={item.id}
...
export default memo(TopOptions);
const styles = css`
  .option1-wrapper {
    display: flex;
    width: 1200px;
