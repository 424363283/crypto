...
export default function partnerChoose() {
  ...
  const chooseList = [
    {
      url: '/static/images/partner/partner-value-activity.svg',
      title: LANG('丰富的线下活动'),
      subTitle: LANG('7*Ymex内部见面会，业内顶级峰会门票，各种体育于娱乐赛事。')
    },
    {
      url: '/static/images/partner/partner-value-support.svg',
      title: LANG('内容素材支持'),
      subTitle: LANG('提供多样的内容素材，助力您的推广宣传。')
    },
    {
      url: '/static/images/partner/partner-value-operation.svg',
      title: LANG('配备专属运营人员'),
      subTitle: LANG('提供专属服务，协助您成为顶级合伙人。')
    },
    {
      url: '/static/images/partner/partner-value-welfare.svg',
      title: LANG('合伙人专属福利'),
  ...
  return (
    <div className={styles.partnerValueAdded}>
      <p className={`${styles.partnerTitle} ${styles.flexDCenter}`}>{LANG('合伙人计划增值服务')}</p>
      ...
      <div className={`${styles.partnerValueAddedBox}`}>
        {chooseList.map((item, idx) => {
          return (
            <div className={styles.partnerValueItem} key={idx}>
