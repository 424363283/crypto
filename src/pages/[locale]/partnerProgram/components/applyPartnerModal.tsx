...
const applyPartnerModal = (props: CancelSettingProps) => {
  ...
  const handleTextArea = (value: string, type: string) => {
    if (value.includes("'OR 1")|| value.includes("OR 1")) {
      setApply({
        ...apply,
        [type]: ''
      });
        ...
        >
          <div className="copy-modal-container">
            ...
            <div className="custom-select-country">
              <SelectCountry noName={true} showLabel={true} hideCode={true} onChange={onCountrySelect} small={false} />
            </div>
...
const copyLeverStyle = css`
  ...
  :global(.copy-cancel-modal) {
    ...
    :global(textarea.ant-input::placeholder) {
      color: var(--text_2);
    }
    :global(.ant-input-data-count ) {
      color: var(--text_2);
