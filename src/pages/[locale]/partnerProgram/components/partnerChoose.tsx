...
export default function partnerChoose() {
  ...
  const chooseList = [
    {
      url: `/static/images/partner/partner-choose-serve${isDark?'-dark':''}.svg`,
      title: LANG('专属服务'),
      subTitle: LANG('7*24小时客服服务，提供全年支持')
    },
    {
      url: `/static/images/partner/partner-choose-trade${isDark?'-dark':''}.svg`,
      title: LANG('多元化交易'),
      subTitle: LANG('玩转交易，轻松躺赢，理财，合约，跟单交易')
    },
    {
      url: `/static/images/partner/partner-choose-security${isDark?'-dark':''}.svg`,
      title: LANG('资金安全保障'),
  ...
  return (
   <div className={styles.partnerChooseModule}>
     <div className={styles.partnerChoose}>
      <p className={`${styles.partnerSecTitle} ${styles.flexDCenter}`}>{LANG('为什么选择YMEX？')}</p>
