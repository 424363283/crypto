...
export default function partnerHeader() {
  ...
  const gotoAgent = () => {
    if (process.env.NODE_ENV === 'development' || window.location?.pathname.indexOf('webdev') > -1) {
      window.open('https://dev-agent.83uvgv.com/zh/agent/home', '_self');
    } else if (process.env.NODE_ENV === 'test' || window.location?.pathname.indexOf('webuat') > -1) {
      window.open('https://webuat-agent.83uvgv.com/zh/agent/home', '_self');
    } else {
      window.open('https://agent.y-mex.com/zh/agent/home', '_self');
    }
  };
      ...
      <div className={`${styles.flexSpace} ${styles.partnerHeaderBox}`}>
        <div className={styles.partnerHeaderLeft}>
          ...
          <div className={`${styles.flexSpace} ${styles.gap16}`}>
            <Button
              ...
              rounded={true}
              style={{ width: isMobile ? '104px' : '256px', fontSize: isMobile ? '16px' : '16px' }}
              onClick={handleApply}
            >
          ...
          <div className={`${styles.flexSpace} ${styles.partnerTrade}`}>
            <div className={styles.partnerTradeItem}>
              <Image
                src={`/static/images/partner/partner-coin${isDark ? '-dark' : ''}.svg`}
                alt=""
            ...
            <div className={styles.partnerTradeItem}>
              <Image
                src={`/static/images/partner/partner-asset${isDark ? '-dark' : ''}.svg`}
                alt=""
                width={isMobile ? 32 : 61}
                height={isMobile ? 32 : 60}
              />
              <div>
        ...
        {!isMobile && (
          <div className={styles.partnerHeaderRight}>
            <Image
              src={`/static/images/partner/partner-pogram${isDark ? '-dark' : ''}.svg`}
              alt=""
