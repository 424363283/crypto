...
export default function SettingPanel() {
 ...
 const getUserInfo = async () => {
  ...
  const switchColor = () => {
    const levelBgColor = ['', 'bg-low', 'bg-middle', 'bg-high', 'bg-higher', 'bg-higher'];
    return levelBgColor[levelIndex];
  };

  const switchLevelColor = () => {
    const levelColor = ['', 'text-low', 'text-middle', 'text-high', 'text-higher', 'text-higher'];
    return levelColor[levelIndex];
  };
  const levelArr = ['', LANG('低'), LANG('中'), LANG('高'), LANG('高'), LANG('高')];

  return (
    <div className="security-setting-wrapper" >
