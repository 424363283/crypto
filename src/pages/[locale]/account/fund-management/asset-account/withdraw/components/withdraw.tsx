...
export const Withdraw = ({ hasQueryCode }: {
  ...
  const createStepsItems = () => {
    ...
    stepItems[stepItems.length] = {
      title: <span className={ clsx(isMobile && step < stepItems.length && 'hidden')}>{LANG('选择网络')}</span>,
    ...
    stepItems[stepItems.length] = {
      title: <span className={ clsx(isMobile && step < stepItems.length && 'hidden')}>{LANG('提币数量')}</span>,
