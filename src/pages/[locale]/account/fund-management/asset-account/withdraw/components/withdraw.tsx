...
export const Withdraw = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      Loading.start();
      await Promise.all([getWithdrawCoinList(), getUser()]);
      Loading.end();
    };
    fetchData();
  }, []);
  ...
  const processChainData = async (cryptoName: string, shouldShowBscAlert = true) => {
    ...
    const res = (await getNetworksChainsApi(cryptoName)) as { data: NetworksChain[] };
  ...
  const onSelectCoin = async (value: number[] | number) => {
    ...
    if (Array.isArray(value)) {
      await processChainData(cryptoName);
      router.replace({ pathname: `/account/fund-management/asset-account/withdraw`, query: { code: cryptoName } });
    }
    Loading.end();
  };
  ...
  const onClickFillAmount = () => {
    setState((draft) => {
      draft.withdrawAmount = String(balance);
    });
  };
  // 需要安全验证提币
