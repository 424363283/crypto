...
const useCopyTradingSwapStroeBase = create<CopyTradingSwapState>()(set => ({
  ...
  bringUserData: {}, // 带单员信息
  searchKey: '', // 搜索key
  isOpenU: true, // 是否开通了合约交易
  isOpenCopy: true, // 是否开启了跟单账户
  ...
  setIsOpenU: (data: boolean) => {
    set(state => ({ isOpenU: data }));
  },
  setOpenCopy: (data: boolean) => {
    set(state => ({ isOpenCopy: data }));
  },
