...
const positionOverlay: OverlayTemplate = {
  ...
  createPointFigures: (args) => {
    // console.log('positionOverlay.createPointFigures called', args);
    const positonExtendData: any = args.overlay.extendData.positionOverlay;
    const volume = args.overlay.extendData.extendsConfig.volume;


    const marginType  = positonExtendData.positionData.marginType; // 1 全仓 2 逐仓
    ...
    // 收益
    const profitLossColor = args.overlay.styles?.profitLossColor;
    const profitLossBackgroundColor = args.overlay.styles?.profitLossBackgroundColor;
    // 持仓量
    ...
    //持仓线
    const positionLineColor = args.overlay.styles?.positionLineColor;


    const positionText = args.overlay.extendData.extendsConfig.profitLoss;
    const changeText = '\ue900';
    const closePositionText = '\ue901';
    const HEIGHT = 20;
    const isLongProfit = args.overlay.extendData.extendsConfig.isLong;
    const PADDING = 4;
   
    const offsetLeft = args.overlay.styles?.offsetLeft ?? 2;
    const { y } = args.coordinates[0];
    const profitLossText = args.overlay.extendData.extendsConfig.direction;
    //  overlay.extendData.extendsConfig.direction;
    const volumeText = `${args.overlay.extendData.volume ?? 0}`;

    const crosshairPoint = args.overlay.extendData.crosshairPoint

    // console.log(overlay.extendData.crosshairPoint)

    // 持仓位置固定
    // console.log('------', positonExtendData.positionData.value)
    const positionPoint = { x: 30, y: args.yAxis.convertToPixel(positonExtendData.positionData.value) };
    ...
    const positionLineFigure = {
      type: 'line',
      attrs: { coordinates: [{x: closePositionBtnFigure.attrs.x + closePositionBtnWidth, y: positionPoint.y}, { x: args.bounding.width, y: positionPoint.y }] },
      styles: {
        // 样式，可选项`solid`，`dashed`
        style: 'solid',
        // 尺寸
      ...
      if (currentCacheTPSLine) {
        ...
        if (!positionExtendData.positionData.stopProfit) {
          if (!positionExtendData.positionData.stopProfit) {
            ...
            e.overlay.extendData.chart.getOverlayById(takeProfitOverlay).setZLevel(10);
        ...
        if (!positionExtendData.positionData.stopLoss) {
          ...
          e.overlay.extendData.chart.getOverlayById(stopLossOverlay).setZLevel(11);
