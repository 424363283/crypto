...
const OriginalKLine: ForwardRefRenderFunction<ChartRef, { containerId?: string }> = (props, ref) => {
  ...
  const containerId = 'bv_kline_chart';
  ...
  const symbolSwapId = window.location.pathname.split('/').pop() || 'btc-usdt';
  const indexToken = (window.location.pathname.split('/').pop() || 'btc-usdt').toUpperCase();

  const isLite = window.location.pathname.includes('lite');

  const pathname = window.location.pathname;

  const positionUnitType = 2;
  ...
  useEffect(() => {
    ...
    // 自动查找并挂载拥有removeOverlay方法的chart实例
    let foundChart = null;
    const widgetAny = widgetRef.current as any;
    for (const key in widgetAny) {
      if (
        widgetAny[key] &&
        typeof widgetAny[key].removeOverlay === 'function'
      ) {
        foundChart = widgetAny[key];
        break;
      }
    }
    if (foundChart) {
      (window as any).chart = foundChart;
      console.log('window.chart挂载成功:', foundChart);
    } else {
      (window as any).chart = widgetRef.current;
      console.log('未找到removeOverlay方法，window.chart挂载为Widget:', widgetRef.current);
    }
    widgetRef.current.setChartType(kLineResolution === 'Time' ? CandleType.Area : originalKLineStyle);
    // 在主K线组件初始化时挂载onOrderdragEnd到window，供overlay调用
    return () => {
      if (widgetRef.current) {
        datafeedRef.current?.cancel();
  ...
  useEffect(() => {
    if (widgetRef.current) {
      if (showPositionLine && isSwapLink) {
        ...
        positionList.forEach((position: any) => {
          if (position?.symbolId === symbolSwapId) {
            ...
            const styles: any = {
              ...
              positionLineColor: isLongProfit ? Color.Green : Color.Red,
            ...
            // 在调用createPositionLine前定义positionOverlayConfig
            const positionOverlayConfig = {
              ...
              positionOverlay: {
                ...
                positionDirectionBtnFigure: { show: true, option: null, styles: { width: 0, height: 0, marginLeft: 0 } },
                positionBtnFigure: { show: true, option: null, styles: { width: 0, height: 0, marginLeft: 0 } },
                positionQtyFigure: { show: true, option: null, styles: { width: 0, height: 0, marginLeft: 0 } },
                changeBtnFigure: { show: true, option: null, styles: { width: 0, height: 0, marginLeft: 0 } },
                closePositionBtnFigure: { show: true, option: null, styles: { width: 0, height: 0, marginLeft: 0 } },
                positionLineFigure: { show: true, option: null, styles: { width: 0, height: 0, marginLeft: 0 } }
              },
              takeProfitOverlay: {
                id: null,
                takeProfitLineFigure: { show: false, option: null, styles: { width: 0, height: 0, marginLeft: 0 } },
                takeProfitBtnFigure: { show: false, option: null, styles: { width: 0, height: 0, marginLeft: 0 } },
                addBtnFigure: { show: false, option: null, styles: { width: 0, height: 0, marginLeft: 0 } }
              },
              stopLossOverlay: {
                id: null,
                stopLossLineFigure: { show: false, option: null, styles: { width: 0, height: 0, marginLeft: 0 } },
                stopLossBtnFigure: { show: false, option: null, styles: { width: 0, height: 0, marginLeft: 0 } },
                addBtnFigure: { show: false, option: null, styles: { width: 0, height: 0, marginLeft: 0 } }
              },
              extendsConfig: {
                profitLoss: `${formatNumber2Ceil(unrealizedPnl, 2)} (${position?.profitRate})`,
                volume: `${total}`,
            ...
            widgetRef.current?.createPositionLine({
              ...
              positionIds: positions.map(item => item.positionId),
              onOrderdrag: (e: any) => {
                console.log('eeeee', e);
              },
              onReverseClick: () => {
        ...
        // showPositionLine为false时，彻底移除所有相关overlay
        widgetRef.current.removeAllPositionLine();
        widgetRef.current.removeNewPositionLine();
      }
    }
    return () => {
      if (widgetRef.current) {
    ...
    if (widgetRef.current && showPositionTPSLLine && isSwapLink) {
      tpSlList?.forEach((position: any) => {
        ...
        if (position?.symbol === symbolSwapId) {
          ...
          const direction = isLong ? LANG('多') : LANG('空'); //持仓方向
          const isLongProfit = (isLong && position?.direction != '1') || (!isLong && position?.direction == '1');
          const profitLoss = isLongProfit ? LANG('止盈') : LANG('止损');

          const closeTooltip = isLongProfit ? LANG('取消止盈') : LANG('取消止损');
          ...
          const total = isLongProfit
            ? `${LANG('预计止盈')}${isLong ? LANG('平空') : LANG('平多')}(${roe}%)`
            : `${LANG('预计止损')}${isLong ? LANG('平空') : LANG('平多')}(${roe}%)`;
          const marginTypeTxt = position.marginType === 1 ? LANG('全仓') : LANG('逐仓');
          widgetRef.current?.createPositionTPSLLine({
            direction,
            styles: {
              marginLineColor: isLongProfit ? Color.Green : Color.Red,
              profitLossBackgroundColor: isLongProfit ? Color.Green : Color.Red,
              profitLossColor,
              expectProfitLossBackgroundColor,
              expectProfitLossColor: isLongProfit ? Color.Green : Color.Red,
              closeColor,
            ...
            onOrderdragEnd: async e => {
              ...
              if (position) {
                ...
                const strategyType =
                  positionSide === 'SHORT' ? (price > priceNow ? '2' : '1') : price > priceNow ? '1' : '2'; // 空仓
      ...
      positionList.forEach((position: any) => {
        if (position.symbolId === symbolSwapId) {
          try {
            const isLong = position?.side === '1';
            const direction = isLong ? LANG('多') : LANG('空'); //持仓方向
            const marinTypeTxt = position.orginalItem.marginType === '1' ? LANG('全仓') : LANG('逐仓');
            ...
            widgetRef.current?.createLiquidationLine({
              ...
              profitLoss: marinTypeTxt + ' ' + direction,
    ...
    if (widgetRef.current && showCurrentEntrustLine && isSwapLink) {
      currentEntrustOrderList?.forEach((position: any) => {
        if (position?.symbol === symbolSwapId) {
          ...
          const isLongProfit = (isLong && position?.direction != '1') || (!isLong && position?.direction == '1');
          const profitLoss = isLongProfit ? LANG('委托开多') : LANG('委托开空');
          const closeTooltip = isLongProfit ? LANG('取消委托') : LANG('取消委托');
