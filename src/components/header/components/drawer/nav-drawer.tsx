...
const menuItems = [
  ...
  },
  {
    ...
    // .filter(item => {
    //   if (item.label === LANG('简易合约')) {
    //     return enableLite && enableH5Lite;
    //   }
    //   return true;
    // })
  },
          ...
          <div className="title-wrap">
            <div className="menu-title" onClick={handleToggle}>
              ...
              <div className="triangle">
                <CommonIcon name={`common-mobile-triangle-${isOpen ? 'up' : 'down'}`} size={20} />
              </div>
            </div>
          </div>
          {isOpen && (
    ...
    <MobileDrawer onClose={onClose} open={open} className="nav-drawer">
      {!isLogin ? (
        <div className="login-register-wrapper">
          <Button rounded outlined hover={false} type='brand' width='100%' size={Size.MD}>
            <TrLink href="/login" className="login-btn">
              {LANG('登录')}
            </TrLink>
          </Button>
          <Button rounded hover={false} type='brand' width='100%' size={Size.MD}>
      ...
      {isLogin && (
        <div className='logout-wrapper'>
          <TrLink href="#" className="logout-btn" native>
            <p onClick={async (evt: any) => {
