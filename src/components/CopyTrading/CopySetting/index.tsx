...
export default function CopySetting() {
  ...
  const fetchConfig = async () => {
    ...
    if (Copy.isLogin()) {
      ...
      if (res?.code === 200) {
        ...
        const showList: any = showObj && Object.values(showObj).map(con => {
          return {
            symbol: con
          };
        });
  ...
  const setDefaultSetting = (shareTrader: any, swap: any) => {
    ...
    const showContract = showList?.length > 0 ? Copy.getObjectIntersection(filterGroup, showList, 'symbol') : filterGroup;
    contractShowList = showContract.map(item => item.alias.toUpperCase());
    reContractList = showContract;
    const swapCoinObj = shareTrader?.data && JSON.parse(shareTrader?.data?.contractInfo);
    const swapCoins: any = swapCoinObj && Object.values(swapCoinObj).map(con => {
      return {
        symbol: con
      };
    });
        ...
        <div className={` ${styles.copySettingContainer}`}>
          <div className={`${styles.all24} ${styles.copySettingLeft}`}>
            ...
            <div className={`${styles.flexSpace} ${styles.my24}`}>
              <div className={`${styles.balance}`}>{copyAsset?.availableBalance?.toFormat(Copy.copyFixed)} USDT</div>
            ...
            <div className={`${styles.setFollow} ${!moreShow.more && styles.gap24}`}>
              ...
              <div
                className={`${styles.flexCenter} ${styles.pointer} ${styles.more}   ${!moreShow.more && styles.closeMore}`}
                onClick={() => {
                  setMoreShow({
                    ...moreShow,
                    more: !moreShow.more
              ...
              <div>
                <div className={`${styles.flexCenter} ${styles.greenLine}`}>
                  <Radio
                    ...
                    size={14}
                  />
                  <span onClick={() => handleTolink()} className={`${styles.textBrand} ${styles.pointer} ${styles.textDashed}`}>
                    {LANG('跟单交易服务条款')}
                  </span>
                </div>
