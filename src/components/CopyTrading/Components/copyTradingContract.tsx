...
const CopyTradingContract = (props: {
  ...
  const setTraderConfig = (arr: any) => {
    ...
    selectOpt &&
      Object.values(selectOpt).map(con => {
        const findContract = con && arr.find(item => item?.symbol?.toUpperCase() === con?.toUpperCase());
  ...
  return (
    <>
      ...
      {
        searchList.length > 0 && <div className={`${styles.setMutitle} ${styles.contractBox}`}>
          {searchList.map(item => {
      ...
      {!searchList.length &&
        <div className={`${styles.contractBox} ${styles.contractEmpty}`}>
          <EmptyComponent />
        </div>
      }
