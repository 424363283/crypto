...
export default function UserInformationPage() {
  ...
  const [cancelActiveShow, setCancelActiveShow] = useState(false);
  ...
  useEffect(() => {
    const checkIdentify = async () => {
      if (tabsActive === CopyTabActive.followerSetting && userType === CopyTradeType.myFllow && !copyActiveType) {
        setTabsActive(CopyTabActive.current);
      }
  ...
  const FollowIncome = () => {
    const [copyPnl, setCopyPnl] = useState({} as COPY_ACCOUNT_ASSET);
    const getCopyPnl = async () => {
      if (!id) return;
      const statisticsParams = {
        uid: id
      };
      const res = await Copy.fetchSharedPnl(statisticsParams);
      if (res?.code === 200) {
        setCopyPnl({
          ...res.data
        });
        ...
        <>
          <div className={`${clsx('incomeRight')} ${!isMobile && clsx('flexSpan')}`}>
            ...
            <div className={clsx('rowLine', isMobile && clsx('mt24'))}>
              ...
              <Button type="brand0" rounded width={136} onClick={() => setTransferModalVisible(true)}>
                {LANG('划转')}
              </Button>
            ...
            <div className={clsx('incomeLeft')}>
              <div>
                ...
                <p
                  ...
                  style={CalibrateValue(copyPnl?.totalPnl?.add(copyPnl?.unRealizedPnl)).color}
                >
                  {CalibrateValue(copyPnl?.totalPnl?.add(copyPnl?.unRealizedPnl), Copy.copyFixed).value}
                </p>
                <p className={clsx('todayIncome')}>
                  ...
                  <span style={CalibrateValue(copyPnl?.dayPnl?.add(copyPnl?.unRealizedPnl)).color}>
                    {CalibrateValue(copyPnl?.dayPnl?.add(copyPnl?.unRealizedPnl), Copy.copyFixed).value}
                  </span>
                </p>
              </div>
            </div>
      ...
      <>
        <div className={clsx('centerfuturesUserAmount')}>
          <div className={clsx('centerAmountListOne')}>
            ...
            <div className={clsx('AmountAvb')}>{copyAsset?.equity?.toFormat(Copy.copyFixed) || 0}</div>
            <div className={clsx('todayProfit')}>
              {LANG('今日盈亏')}
              <span className={clsx('ml-4')} style={CalibrateValue(copyPnl?.dayPnl?.add(copyPnl?.unRealizedPnl)).color}>
                {CalibrateValue(copyPnl?.dayPnl?.add(copyPnl?.unRealizedPnl), Copy.copyFixed).value}（
                {CalibrateValue(copyPnl.profitRate?.mul(100), Copy.copyFixed).value}%）
              </span>
            </div>
          </div>
          <div className={clsx('centerfuturesUserList')}>
            <p className={clsx('centerfuturesUserListTitle')}>{LANG('钱包余额')}(USDT)</p>
            <span>{copyAsset?.accb?.toFormat(Copy.copyFixed) || 0}</span>
          </div>
          <div className={clsx('centerfuturesUserList')}>
            <p className={clsx('centerfuturesUserListTitle')}>{LANG('未实现盈亏')}(USDT)</p>
            <span style={CalibrateValue(unrealised).color}>
              {CalibrateValue(
                formatNumber2Ceil(copyPnl?.unRealizedPnl, Copy.copyFixed, false).toFixed(Copy.copyFixed)
              ).value?.toFormat(Copy.copyFixed)}
            </span>
          </div>
          <div className={clsx('centerfuturesUserList')}>
            <p className={clsx('centerfuturesUserListTitle')}>{LANG('可用保证金')}(USDT)</p>
            <span>{copyAsset.availableBalance?.toFormat(Copy.copyFixed) || 0}</span>
          </div>
          <div className={clsx('centerfuturesUserList')}>
            <Button
              height={40}
  ...
  const FollowMarginModule = () => {
    ...
    return (
      <div className={`${clsx('incomeBox')} ${!isMobile ? clsx('mt40') : ''}`}>
        <div className={`${clsx('incomeRight')} ${!isMobile && clsx('flexSpan')}`}>
          <div className={clsx('girdRow5', localLang === 'en' && 'girdRow5En')}>
            <div className={clsx('rowLine')}>
              <p className={clsx('mb16', localLang === 'en' && 'incomeTitle')}>{LANG('跟单保证金占用')}(USDT)</p>
              <p className={clsx('incomeValue')}>{copyPnl?.positinMargin?.toFixed(Copy.copyFixed) || 0}</p>
            </div>
            <div className={clsx('rowLine')}>
              <p className={clsx('mb16', localLang === 'en' && 'incomeTitle')}>{LANG('预估净收益')}(USDT)</p>
              <p
                className={`${clsx('incomeValue')}`}
                style={CalibrateValue(copyPnl?.totalPnl?.add(copyPnl?.unRealizedPnl)).color}
              >
                {CalibrateValue(copyPnl?.totalPnl?.add(copyPnl?.unRealizedPnl), Copy.copyFixed).value}
              </p>
            ...
            <div className={clsx('rowLine')}>
              <p className={clsx('mb16', localLang === 'en' && 'incomeTitle')}>{LANG('今日收益')}(USDT)</p>
              <p
                className={`${clsx('incomeValue')}`}
                style={CalibrateValue(copyPnl?.dayPnl?.add(copyPnl?.unRealizedPnl)).color}
              >
                {CalibrateValue(copyPnl?.dayPnl?.add(copyPnl?.unRealizedPnl), Copy.copyFixed).value}
              </p>
            </div>
            <div className={clsx('rowLine')}>
              <p className={clsx('mb16', localLang === 'en' && 'incomeTitle')}>{LANG('分润比例')}</p>
              <p className={clsx('incomeValue')}>{traderInfo?.shareRoyaltyRatio?.mul(100)}%</p>
            </div>
            <div className={clsx('rowLine')}>
              <p className={clsx('mb16', localLang === 'en' && 'incomeTitle')}>{LANG('跟单时间')}</p>
      ...
      <>
        {detailInfo.isShowFollowBtn && (
          <Button
            ...
            onClick={() => {
              if (isFulled) return;
              if (!isOpenU || !isOpenCopy) {
                setCancelActiveShow(true);
                return;
              }
    ...
    <div className={`${clsx('centerfuturesContent')} ${!detailInfo.isShowAsset && !isMobile && clsx('mb80')}`}>
      <div className={clsx('centerfuturesUserBox')}>
        <div className={clsx('centerfuturesHeader')}>
          ...
          <span
            className={`${
              !detailInfo.subTitle ? clsx('centerfuturesActive') : clsx('centerfuturesInActive', 'centerHoverTitle')
            } 
            `}
                  ...
                  <div className={clsx('descriptionBox')}>
                    <p
                      className={`${clsx('description')} ${
                        traderInfo?.description?.length >= 200 && showMore && clsx('descriptionDellipsis')
                      }`}
                    >
                      {traderInfo?.description}
                    </p>
                    {traderInfo?.description?.length >= 200 && (
                      <span className={clsx(showMore ? 'arrowDown' : 'arrowUp')}>
                        <CommonIcon
                          onClick={() => {
      ...
      content={!isOpenU ? '开启跟单交易前，请先确认开通合约交易。':'跟单交易前，请先确认开启跟单账户。'}
        isOpen={cancelActiveShow} close={() => setCancelActiveShow(false)} />
      <style jsx>{styles}</style>
    </div>
  );
}
const styles = css`
  ...
  .incomeBox {
    ...
    .girdRow5En {
      grid-template-columns: 221px 157px 163px auto auto;
      gap: 68px;
      @media ${MediaInfo.mobile} {
        grid-template-columns: repeat(1, 1fr);
      }
