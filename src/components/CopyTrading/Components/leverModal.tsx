...
const LeverModal = (props: LeverModalProps) => {
  ...
  const onChangeModel = (value: { label: string; value: number }[]) => {
    ...
    if (pair.value === 2 || pair.value === 3) {
      const filterOpt = leverModelOpts.filter((item) => item.value !== 1)
      model.leverModel = filterOpt[0]
    } else {
      model.leverModel = leverModelOpts[0]
    }
    setLever({
      ...lever,
      ...model
  ...
  const MarginAndModule = () => {
    ...
    const handleConfrim = () => {
      ...
      if (!lever.maginModel?.value || !lever.leverModel?.value) {
        return message.error(LANG('请选择保证金&杠杆模式'))
      }
      if (lever.leverModel.value === 2 && !leverageLevel) {
        return message.error(LANG('请输入固定杠杆'))
      }
      const filterMargin = copyMargin?.filter((item) => !item.value)
      if (lever.leverModel.value === 3 && filterMargin && filterMargin?.length > 0) {
        return message.error(LANG('请输入自定义杠杆'))
      }
      ...
      if (lever.maginModel?.value === 1 && lever.leverModel?.value !==1) {
        return message.error(LANG('保证金模式为交易员模式，杠杆模式只能是交易员模式'))
      }
      confrim(prams);
      close();
    };
    ...
    const leverModelFilter = useMemo(() => {
      // 跟单员设置跟单配置，保证金和杠杆模式调整，保证金模式选择：跟随交易员，杠杆模式：跟随交易员，保证金模式选择：全仓/逐仓，杠杆模式：固定杠杆/自定义杠杆
    ...
    return (
      <div className="copy-modal-container">
        ...
        <Select
          ...
          width={138}
          bgColor={'var(--fill_3)'}
          borderColor={!lever.maginModel ? 'var(--color-red)' : ''}
          height={48}
        ...
        <Select
          ...
          borderColor={!lever.leverModel ? 'var(--color-red)' : ''}
        ...
        <Button type="primary" className={!isMobile ? 'mt24' : 'mb24'} rounded height={48} width={'100%'} onClick={handleConfrim}>
          {LANG('确定')}
        </Button>
        <style jsx>{copyLeverStyle}</style>
      </div>
