...
export default function CopyTradingData(props: { overviewData: OVERVIEW_DATA; bringData: BRING_DATA; cycle: number }) {
  ...
  const tradePercent = useMemo(() => {
    ...
    return {
      ...
      victoryRate: copyBringData[`victoryRate${cycle}`] || 0,
      profitRate: copyBringData[`profitRate${cycle}`] || 0,
      profitOrderNumber: copyBringData[`profitOrderNumber${cycle}`] || 0,
      lossOrderNumber: copyBringData[`lossOrderNumber${cycle}`] || 0,
      profitLossRate: copyBringData[`profitLossRate${cycle}`] || 0,
      avgLoss: copyBringData[`avgLoss${cycle}`] || 0,
      avgProfit: copyBringData[`avgProfit${cycle}`] || 0,
      profitAmount: copyBringData[`profitAmount${cycle}`] || 0,
      winRate: !percentWin || percentWin === 'NaN'   ? 0 : percentWin?.mul(100).toFixed(Copy.copyFixed) 
    };
  }, [copyOverviewData]);
  return (
    <>
      <div className={clsx('all24', 'gap24', 'h379')}>
        ...
        <div className={clsx('flexSpace', 'incomeBox')}>
          <div>
            <p className={clsx('incomeCount', 'mb8')} style={CalibrateValue(tradePercent?.profitRate).color}>
          ...
          <div>
            <p className={clsx('incomeCount', 'mb8')} style={CalibrateValue(tradePercent?.profitAmount).color}>
        ...
        <div className={clsx('flexSpace')}>
          ...
          <span className={clsx('textPrimary')}>{tradePercent?.winRate}%</span>
        </div>
        <div>
