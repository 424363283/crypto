...
export default function BringSetting() {
  ...
  const toggleShareStatus = async () => {
    ...
    const res = await Copy.fetchUpdateShareConfig({
      uid: user?.uid,
      shareStatus: settingInfo.shareStatus === 1 ? 0 : 1
    });
    Loading.end();
    if (res?.code === 200) {
  ...
  return (
    <div className={styles.bringSettingBox}>
      <div className={styles.flexSpace}>
        ...
        <div>
          <Switch aria-label="" onChange={toggleShareStatus} checked={settingInfo.shareStatus === 1} />
        </div>
      </div>
      <div className={`${styles.tradeRow} ${styles.mt24}`}>
        <div className={styles.tradeRow2}>
          <div>{LANG('个人昵称')}</div>
          <div className={`${isMobile ? styles.wh5Overflow : styles.wOverflow}`}>{showNickname}</div>
        </div>
        <div className={styles.textRight}>
          {nickNameInfo.status === 0 && (
            <Button type={'yellow10'} rounded width={104}>
              {LANG('审核中')}
            </Button>
          )}
          {nickNameInfo.status !== 0 && <>
            ...
            >
              {nickNameInfo.status === 2 && !isMobile &&
                <p >
                  <p className={styles.failTip}>{nickNameInfo.nickname}</p>
                  <p className={`${styles.flexCenter} ${styles.gap8} ${styles.nickNameTips}`}>
      ...
      {nickNameInfo.status !== 0 && nickNameInfo.status === 2 && isMobile &&
          <p >
            <p className={styles.failTip}>{nickNameInfo.nickname}</p>
            <p className={`${styles.flexCenter} ${styles.gap8} ${styles.nickNameTips}`}>
              <CommonIcon size={12} className="icon" name="common-question-outline-red-0" />
      ...
      <div className={`${styles.tradeRow} ${styles.mt24}`}>
        <div className={styles.tradeRow2}>
          <div>{LANG('带单合约')}</div>
          <div className={`${isMobile ? styles.wh5Overflow : styles.wOverflow}`}>{showContract}</div>
        </div>
        <div className={styles.textRight}>
      ...
      <div className={styles.tradeRow}>
        <div className={styles.tradeRow2}>
          ...
          <div className={`${isMobile ? styles.wh5Overflow : styles.wOverflow}`}>{settingInfo.copyMinAvailableMargin}</div>
          {isMobile && (
      ...
      <div className={styles.tradeRow}>
        <div className={styles.tradeRow2}>
          <div>{LANG('个人简介')}</div>
          <div className={`${isMobile ? styles.wh5Overflow : styles.wOverflow}`}>{showContent}</div>
        </div>
        <div className={styles.textRight}>
          ...
          {contentInfo.status !== 0 && (
            ...
            >
              {contentInfo.status === 2 && !isMobile &&
                <p>
                  <p className={styles.failTip}>
                    {contentInfo?.content}
                  </p>
      ...
      {contentInfo.status !== 0 &&contentInfo.status === 2 && isMobile &&
        <p>
          <p className={styles.failTip}>
            {contentInfo?.content}
          </p>
