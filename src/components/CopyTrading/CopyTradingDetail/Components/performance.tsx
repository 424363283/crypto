...
export default function CopyTradingPreformance(props: { tabKey: string }) {
  ...
  const fetchTraderList = async (follower:any) => {
    // const follower = await Copy.getCopyTraderList({ lUid: id, page: 1, size: 10 });
  ...
  const fetchHistoryList = async (res:any) => {
    ...
    if (res.code === 200) {
      ...
      data.forEach(pnl => {
        const diffTime = pnl.mtime.sub(pnl.ctime)
  ...
  const fetchAllData = useCallback(async () => {
    ...
    const params: any = {
      ...
      const [
        ...
        Copy.getCopyTraderList({ lUid: id, page: 1, size: 10 }),
