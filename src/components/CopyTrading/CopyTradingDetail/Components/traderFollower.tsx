...
export default function TraderFollower() {
  ...
  const ProfitCom = (props: { item }) => {
    ...
    const calMargin = useMemo(() => {
      ...
      const currentMargin = item?.positionMargin
      const currentProfit = item?.unRealizedPnl
      return {
        marginValue: currentMargin.toFixed(Copy.copyFixed),
        profitValue: item?.totalPnl?.add(currentProfit).toFixed(Copy.copyFixed),
        todayProfit: item.dayPnl?.add(currentProfit)?.toFixed(Copy.copyFixed)
      };
    }, []);
    return (
      <>
                ...
                return (
                  <div className={clsx('mpRow')} key={item.id}>
                    <div className={clsx('flexSpace', 'mpTop')}>
                      ...
                      <div className={clsx('date')}> {item?.copyTime && dayjs(item?.copyTime).format('YYYY-MM-DD HH:mm')}</div>
                    </div>
                    ...
                    <div className={clsx('flexSpace')}>
                      ...
                      <div className={clsx('mpValue')}> {item.totalPnl?.add(item.unRealizedPnl).toFixed(Copy.copyFixed)}</div>
                    </div>
                    {userType === CopyTradeType.myFllow && (
                      <div className={clsx('textRight', 'flexSpace', 'gap8')}>
                        <CopyBtn btnTxt={LANG('取消跟单')} btnType="gracy" width={'50%'} />
                        <TrLink
                          style={{width: '50%'}}
                          href={`/copyTrade/setting/${item.luid}`}
                          query={{
                        ...
                        >
                          <CopyBtn btnTxt={LANG('详情')} width={'100%'} />
                        </TrLink>
                      </div>
                    )}
                    {userType === CopyTradeType.myBring && (
                      <div>
                        <CopyBtn  onClick={() => handleRemove(item)} btnTxt={LANG('移除')} btnType="gracy" width={isMobile ? '100%' : 104} />
                      </div>
                    )}
                  </div>
                );
                  ...
                  {followData.map((item, idx) => {
                    return (
                      <div className={`${!showAction ? clsx('tradeRow') : clsx('tradeRow5')}`} key={item.id}>
                        ...
                        <div className={clsx('flexCenter')}>{item.equity?.toFixed(Copy.copyFixed)}</div>
                        <div
                          className={` ${!showAction && clsx('textRight')} ${userType === CopyTradeType.traderDetail ? clsx('flexEnd') : clsx('flexCenter')}`}
