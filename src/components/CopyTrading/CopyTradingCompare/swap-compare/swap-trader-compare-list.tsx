...
const SwapCopyTradingCompare = forwardRef((props, ref) => {
  ...
  const findMaxAndPosition = (formatArr: any) => {
    const len = compareIds.length
    const arr = formatArr.slice(0,len).map(item => (!item || item === 'Infinity' || item === "NaN" ? 0 : Number(item)));
  ...
  const transactionData = [
    ...
    {
      key: '4',
      property: LANG('ProfitLoss'),
      trader1: `${compareTraders[0]?.copyTradeUserStatisticsSummaryVo[`avgProfit${date}`]?.toFixed(Copy.copyFixed) || ''
        } / ${compareTraders[0]?.copyTradeUserStatisticsSummaryVo[`avgLoss${date}`]?.toFixed(Copy.copyFixed) || ''}`,
      trader2: `${compareTraders[1]?.copyTradeUserStatisticsSummaryVo[`avgProfit${date}`]?.toFixed(Copy.copyFixed) || ''
        } / ${compareTraders[1]?.copyTradeUserStatisticsSummaryVo[`avgLoss${date}`]?.toFixed(Copy.copyFixed) || ''}`,
      trader3: `${compareTraders[2]?.copyTradeUserStatisticsSummaryVo[`avgProfit${date}`]?.toFixed(Copy.copyFixed) || ''
        } / ${compareTraders[2]?.copyTradeUserStatisticsSummaryVo[`avgLoss${date}`]?.toFixed(Copy.copyFixed) || ''}`,
      trader4: `${compareTraders[3]?.copyTradeUserStatisticsSummaryVo[`avgProfit${date}`]?.toFixed(Copy.copyFixed) || ''
            ...
            {baseData.map(base => {
              return (
                <div key={base.key} className={styles.swapTraderRow5}>
                  ...
                  <div
                    className={`${styles.swapTraderItem} ${base?.highlightIndex && base?.highlightIndex?.includes(1) && styles.swapTraderhight
                      }`}
                  >
                    {base?.trader1}
                  </div>
                  <div
                    className={`${styles.swapTraderItem} ${ base?.highlightIndex&& base?.highlightIndex?.includes(2) && styles.swapTraderhight
                      }`}
                  >
                    {base.trader2}
                  </div>
                  <div
                    className={`${styles.swapTraderItem} ${base?.highlightIndex&&base?.highlightIndex?.includes(3) && styles.swapTraderhight
                      }`}
                  >
                    {base.trader3}
                  </div>
                  <div
                    className={`${styles.swapTraderItem} ${base?.highlightIndex && base?.highlightIndex?.includes(4) && styles.swapTraderhight
                      }`}
                  >
                    {base.trader4}
                  </div>
              ...
              {transactionData.map((base, idx) => {
                return (
                  <div key={base.key} className={styles.swapTraderRow5}>
                    ...
                    <div
                      className={`${styles.swapTraderItem} ${idx === 0 ? (base.trader1 > 0 ? styles.swapTraderGreen : styles.swapTraderRed) : ''
                        }  ${base?.highlightIndex && base?.highlightIndex?.includes(1) && styles.swapTraderhight}`}
                    >
                      {base.trader1}
                    </div>
                    <div
                      className={`${styles.swapTraderItem} ${idx === 0 ? (base.trader1 > 0 ? styles.swapTraderGreen : styles.swapTraderRed) : ''
                        } ${base?.highlightIndex && base?.highlightIndex?.includes(2) && styles.swapTraderhight}`}
                    >
                      {base.trader2}
                    </div>
                    <div
                      className={`${styles.swapTraderItem} ${idx === 0 ? (base.trader1 > 0 ? styles.swapTraderGreen : styles.swapTraderRed) : ''
                        } ${base?.highlightIndex && base?.highlightIndex?.includes(3) && styles.swapTraderhight}`}
                    >
                      {base.trader3}
                    </div>
                    <div
                      className={`${styles.swapTraderItem} ${idx === 0 ? (base.trader1 > 0 ? styles.swapTraderGreen : styles.swapTraderRed) : ''
                        } ${base?.highlightIndex && base?.highlightIndex?.includes(4) && styles.swapTraderhight}`}
                    >
                      {base.trader4}
                    </div>
                  </div>
