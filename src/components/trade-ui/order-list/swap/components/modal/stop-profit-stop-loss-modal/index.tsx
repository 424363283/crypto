...
import { cacheTPSLineFigureKey } from '@/components/YKLine/StockChart/OriginalKLine/extension/overlays/positionOverlay';
...
const StopProfitStopLossModal = ({
  ...
  useEffect(() => {
    ...
    Utils.setDefaultSpsl({ data, incomeLoss: false, isEdit });
    Utils.setDefaultSpsl({ data, incomeLoss: true, isEdit });
  ...
  const handleClose = () => {
    ...
    if (positionId && window.removeOverlayIfExists) {
      const tpslLine = cacheTPSLineFigureKey[positionId];
      if (tpslLine) {
        window.removeOverlayIfExists(tpslLine.takeProfitOverlayId);
        window.removeOverlayIfExists(tpslLine.stopLossOverlayId);
      }
    }
    onClose?.();
  };
      ...
      <div className={clsx('content')}>
        <div className={clsx('symbol-info')}>
          <div className={clsx('symbol-name')}>
            ...
            <div className={clsx('wallet')}>{LANG(Swap.Assets.walletFormat(walletData)?.alias)}</div>
          </div>
                    ...
                    <div className={clsx('liquidation-pannel')}>
                      ...
                      <div className={clsx('liquidation-form', !isLimit ? 'disabled' : '')}>
                        <InputPrice
                          ...
                          // select
                          showSuffix={false}
                          placeholder={!isLimit ? '' : LANG('请输入价格')}
                          unit={volumeUnit}
                          disabled={!isLimit}
                          value={!isLimit ? LANG('市价') : price ? price : ''}
                          data={data}
                      ...
                      <div className={clsx('liquidation-pannel')}>
                        ...
                        <div className={clsx('liquidation-form', !store.stopIsLimit ? 'disabled' : '')}>
                          <InputPrice
                            ...
                            showSuffix={false}
                            // select
                            placeholder={!store.stopIsLimit ? '' : LANG('请输入价格')}
                            unit={priceUnitText}
                            disabled={!store.stopIsLimit}
                            value={!store.stopIsLimit ? LANG('市价') : stopLossLimitPrice ? stopLossLimitPrice : ''}
