...
import AffiliateSelect from '@/components/select/affiliate-select';
import { ResultModal } from '@/components/strategy/spot-strategy-result-modal';
...
import { LOCAL_KEY, localStorageApi, SESSION_KEY, useAppContext } from '@/core/store';
...
import { CreateInvestConfirmModal } from '../../../../strategy/create-invest-confirm-modal';
import ExchangeIcon from '../../spot-pro/components/exchange-icon';
import { BaseModalStyle } from './base-modal-style';
import Input from './input';

const { Position, Strategy, Invest } = Spot;
...
const InvestView = () => {
  ...
  const { investDataList, period, title, investSymbolList, investSymbolStringList, investSymbolMap } = Invest.state;
  ...
  useEffect(() => {
    ...
    if (from === 'trading-bot-invest' && data) {
      Invest.changeInvestDataList(data?.investList);
      Invest.changeInvestSymbolStringList(data?.investSymbolStringList);
      Invest.setPeriod(data?.period);
  ...
  const onAddCoinClicked = () => {
    ...
    Invest.changeInvestDataList(cloneInvestDataList);
    Invest.changeInvestSymbolStringList(cloneInvestSymbolStringList);
  };
  ...
  const onConfirmCreate = async () => {
    ...
    if (stringList.length <= 2) {
      newTitle = `${stringList.join(',')}${LANG('现货定投')}`;
    }
    if (stringList.length > 2) {
    ...
    const result = await Invest.createInvest(payload);
    if (result.code === 200) {
      setSuccess(true);
      Strategy.getBalance();
      await Position.fetchSpotInvestPositionList(LoadingType.Hide);

      const trackParams: any = {
        invest_coin: 'USDT',
        tradepair_list: investDataList.map((item) => item.symbol),
        invest_interval: INVEST_TIME.find((item) => item.value === period)?.description,
        each_issue_amount: oneCycleTotal + '',
        invest_times: Math.floor(Number(balance.div(oneCycleTotal))),
      };

      EVENT_TRACK(EVENT_NAME.SPOT_INVEST_SUBMIT, trackParams);

      investDataList.map((item) => {
        EVENT_TRACK(EVENT_NAME.SPOT_INVEST_SUBMIT_DETAIL, {
          invest_coin: 'USDT',
          coin_name: item.symbol,
          coin_invest_amount: item.amount + '',
          invest_interval: INVEST_TIME.find((item) => item.value === period)?.description,
        } as any);
      });
    } else {
  ...
  const onRankCopyClicked = (index: number) => {
    ...
    Invest.changeInvestDataList(item.symbols.map((item: any) => ({ symbol: item.symbol, amount: item.amount })));
    Invest.changeInvestSymbolStringList(item.symbols.map((item: any) => item.symbol));
    Invest.setPeriod(item.period);
  };

  const onCancel = useCallback(() => {
    setTutorialModelVisible(false);
  }, []);
      ...
      <div className='invest-container'>
        ...
        <div className='step-wrapper'>
                ...
                <li key={item.symbol + index}>
                  <AffiliateSelect
                    ...
                    onChange={(val) => {
                      ...
                      Invest.changeInvestDataList(list);
                      Invest.changeInvestSymbolStringList(stringList);
                    }}
          ...
          <div className='step step3'>
            <div>3. {LANG('定投间隔')}</div>
            <div className='info'>
              <AffiliateSelect value={period} onChange={(val) => Invest.setPeriod(val as number)} list={INVEST_TIME} />
              <div className='label'>
                {LANG('每期总额')}
          ...
          <div className='step step4'>
            ...
            <input
              ...
              value={title}
              onChange={(e) => {
                Invest.setTitle(e.target.value);
              }}
            />
          </div>
          <button className='create-btn' onClick={onCreateClicked}>
            {isLogin ? LANG('创建') : LANG('登录后创建')}
          </button>
        </div>
      </div>
      <div
