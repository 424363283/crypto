...
const TimeSelection: FC<TimeSelectionProps> = (props) => {
  ...
  const { setResolutionList, resolutionList, intl } = props;
    ...
    if (editBtn) {
      ...
      if (!findResolution) {
        ...
        if (newTempResolutionList.length < 1) {
          message.warning(intl.formatMessage({ id: 'future.tradingView.time.selectwarning' }));
        } else {
          setTempResolutionList(newTempResolutionList);
        }
      }
  ...
  //           <div
  //             onClick={() => setTimeModel(!timeModel)}
  //             className={classnames(styles.KlineSelectIcon, timeModel && styles.arrowUp)}>
  //             <BVIcon.KlineSelectIcon />
  //           </div>
  //           {timeModel && <div className={styles.mobileTimeInfoContent}>{content}</div>}
  //         </div>
  //       )
  //     }
  //     {timeModel && <div className={styles.maskContent}></div>}
  //   </div>
  // );
};
